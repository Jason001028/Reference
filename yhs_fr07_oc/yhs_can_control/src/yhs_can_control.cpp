...
bool if_straight_bool = false;
//直线控制回调函数
void CanControl::if_straightCallBack(const std_msgs::Bool if_straight){
    if(if_straight.data){
        if_straight_bool = true;
    }else{
        if_straight_bool = false;
    }
}
//速度控制回调函数
void CanControl::ctrl_cmdCallBack(const geometry_msgs::Twist msg){
  unsigned int brake = 0;
  unsigned int gear;
  float tmp_angular = (msg.angular.z * 180) / M_PI;
  double ratio = ratio_higher;
  ...
  short angular = tmp_angular * 100 * ratio;
  ...
  sendData_u_vel_[4] = sendData_u_vel_[4] | (0xf0 & ((brake & 0x0f) << 4));
...
//急停控制回调函数
void CanControl::lock_wheelCallBack(const std_msgs::Bool lock_wheel)
{
  if(!lock_wheel.data){
    isStop = false;
  }else{
...
//加速控制回调函数
void CanControl::acc_velCallBack(const yhs_can_msgs::CarAccelerate acc){
  if(acc.isAcc){
    isAcc = true;
    acc_vel = acc.accRange + 1;
    if (acc_vel > 1.4){
        acc_vel = 1.4;
    }
    acc_ang = (1.0 - acc.accRange);
  }else{
    isAcc = false;
  }
}
			...
			for(int j=0;j<1;j++)
			{
				...
				switch (recv_frames_[0].can_id)
				{
					...
					case 0x98C4EAEF:
					{
						...
						if(0x01 & recv_frames_[0].data[1]) msg.Veh_fb_STMCUDisOnline = true;	else msg.Veh_fb_STMCUDisOnline = false;

						if(0x02 & recv_frames_[0].data[1]) msg.Veh_fb_STEncoderDisOnline = true;	else msg.Veh_fb_STEncoderDisOnline = false;

						if(0x04 & recv_frames_[0].data[1]) msg.Veh_fb_STOverCurrent = true;	else msg.Veh_fb_STOverCurrent = false;

						if(0x10 & recv_frames_[0].data[2]) msg.Veh_fb_BraMCUDisOnlie = true;	else msg.Veh_fb_BraMCUDisOnlie = false;

						if(0x20 & recv_frames_[0].data[2]) msg.Veh_fb_BraOverCurrent = true;	else msg.Veh_fb_BraOverCurrent = false;
...
void CanControl::run()
{
  ctrl_cmd_sub_ = nh_.subscribe<geometry_msgs::Twist>("cmd_vel", 5, &CanControl::ctrl_cmdCallBack, this);
	io_cmd_sub_ = nh_.subscribe<yhs_can_msgs::io_cmd>("io_cmd", 5, &CanControl::io_cmdCallBack, this);
  lock_wheel_sub_ = nh_.subscribe<std_msgs::Bool>("lock_wheel", 5, &CanControl::lock_wheelCallBack, this);
    if_straight_sub_ = nh_.subscribe<std_msgs::Bool>("if_straight", 5, &CanControl::if_straightCallBack, this);
  acc_vel_sub_ = nh_.subscribe<yhs_can_msgs::CarAccelerate>("acc_vel", 5, &CanControl::acc_velCallBack, this);
  ctrl_fb_pub_ = nh_.advertise<yhs_can_msgs::ctrl_fb>("base_feedback",5);
